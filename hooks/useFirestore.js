...
export function useFirestore() {
  ...
  /**
   * Fetch ALL receipts — plain array, no pagination.
  ...
  /**
   * Alias kept for any component that still calls getAllReceipts().
   */
  const getAllReceipts = async (flatId = null) => getReceipts(flatId);
  ...
  const getDashboardStats = async () => {
    if (!user) return {};
    const [flats, allReceipts] = await Promise.all([getFlats(), getReceipts()]);
