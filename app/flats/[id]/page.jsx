...
import {
  ...
  Download, Trash2, Plus, IndianRupee, MoreVertical
} from 'lucide-react';
        ...
        <div className="max-w-4xl space-y-4 sm:space-y-6">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h2
                      className="text-white text-lg sm:text-xl font-semibold truncate"
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-gray-100 gap-2">
              <h3
                className="font-semibold text-[#1a1a2e] text-base sm:text-lg"
                style={{ fontFamily: 'Playfair Display, serif' }}
              >
                Receipt History
              </h3>
            ...
            {receipts.length === 0 ? (
              ...
              <>
                {/* ── Mobile: receipt cards (hidden on md+) ── */}
                <div className="flex flex-col divide-y divide-gray-50 md:hidden">
                  {receipts.map((r) => (
                    <div key={r.id} className="px-4 py-4 flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-semibold text-[#1a1a2e] text-sm">{r.month}</span>
                          <span className="font-bold text-[#1a1a2e] text-sm">
                            {currency}{Number(r.paidAmount || 0).toLocaleString('en-IN')}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 flex-wrap text-xs text-gray-400">
                          <span className="capitalize">{r.modeOfPayment}</span>
                          {r.paymentDate && <span>· {r.paymentDate}</span>}
                          {r.receiptNumber && <span className="font-mono">· #{r.receiptNumber}</span>}
                        </div>
                        {r.remarks && (
                          <p className="text-xs text-gray-400 italic truncate">{r.remarks}</p>
                        )}
                      </div>
                  ...
                  {/* Mobile: Download All footer */}
                  {receipts.length > 0 && (
                    <div className="px-4 py-3">
                      <button
                        onClick={downloadAll}
                        className="flex items-center justify-center gap-2 w-full py-2.5 text-sm text-[#b8861f] font-medium border border-[#e2b04a]/40 rounded-xl hover:bg-[#fdf6ec] transition-colors"
                      >
                ...
                <div className="hidden md:block overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {receipts.map((r) => (
                        <tr key={r.id} className="border-t border-gray-50 hover:bg-[#fdf6ec]/40 transition-colors">
