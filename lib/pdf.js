...
export function generateReceiptPDF(settings, flat, receipts, startReceiptNo = 1) {
  ...
  groups.forEach((groupReceipts, receiptNumber) => {
    ...
    // For multi-month: "... for the months from <firstMonth> to <lastMonth> by <mode>"
    // For single-month: "... for the month of <month> by <mode>"
    // Only actual data values (name, flat no, amount words, month names, mode) are dynamic/blue.
    // All connecting words stay static/dark.
